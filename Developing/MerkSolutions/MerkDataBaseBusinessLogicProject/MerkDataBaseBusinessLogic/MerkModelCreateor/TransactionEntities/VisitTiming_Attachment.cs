...
{
	public partial class VisitTiming_Attachment : DBCommon, IDBCommon, IPEMR_Element
	{
		MerkFinanceEntities _context;
		private static List<VisitTiming_Attachment> _items;
		public static List<VisitTiming_Attachment> ItemsList = new List<VisitTiming_Attachment>();
		...
		public override bool LoadItemsList()
		{
			...
			ItemsList = DBContext_External.VisitTiming_Attachment.ToList();
			return true;
		}

		public int TableIdentityID
		{
			get { return (int)DB_TableIdentity.VisitTiming_Attachment; }
		}
		...
		{
			get { return "VisitTiming_Attachment"; }
		}

		public IDBCommon GetSpecificEntity(MerkFinanceEntities context, int id)
		{
			return context.VisitTiming_Attachment.FirstOrDefault(item => item.ID.Equals(id));
		}
		...
		{
			get { return DB_PEMR_ElementType.VisitTIming_Attachment; }
		}
			...
			get
			{
				...
				if (PEM_ElementPrintOrder_cu.ItemsList.Count == 0)
				{
					element = PEMR_Elemet_p.ItemsList.Find(
						item => Convert.ToInt32(item.ID).Equals((int)DB_PEMR_ElementType.VisitTIming_Attachment));
					if (element != null)
				...
				PEM_ElementPrintOrder_cu elementPrintOrder = PEM_ElementPrintOrder_cu.ItemsList.Find(
					item => Convert.ToInt32(item.PEMR_Elemet_P_ID)
						.Equals((int)DB_PEMR_ElementType.VisitTIming_Attachment));

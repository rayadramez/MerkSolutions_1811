...
{
	public partial class VisitTiming_EOMSign : DBCommon, IDBCommon, IPEMR_Element
	{
		MerkFinanceEntities _context;
		private static List<VisitTiming_EOMSign> _items;
		public static List<VisitTiming_EOMSign> ItemsList = new List<VisitTiming_EOMSign>();
		...
		public override bool LoadItemsList()
		{
			...
			ItemsList = DBContext_External.VisitTiming_EOMSign.ToList();
			return true;
		}

		public int TableIdentityID
		{
			get { return (int)DB_TableIdentity.VisitTiming_EOMSign; }
		}
		...
		{
			get { return "VisitTiming_EOMSign"; }
		}

		public IDBCommon GetSpecificEntity(MerkFinanceEntities context, int id)
		{
			return context.VisitTiming_EOMSign.FirstOrDefault(item => item.ID.Equals(id));
		}
		...
		{
			get { return DB_PEMR_ElementType.VisitTiming_EOMSign; }
		}
			...
			get
			{
				...
				if (PEM_ElementPrintOrder_cu.ItemsList.Count == 0)
				{
					element = PEMR_Elemet_p.ItemsList.Find(
						item => Convert.ToInt32(item.ID).Equals((int)DB_PEMR_ElementType.VisitTiming_EOMSign));
					if (element != null)
				...
				PEM_ElementPrintOrder_cu elementPrintOrder = PEM_ElementPrintOrder_cu.ItemsList.Find(
					item => Convert.ToInt32(item.PEMR_Elemet_P_ID)
						.Equals((int)DB_PEMR_ElementType.VisitTiming_EOMSign));
